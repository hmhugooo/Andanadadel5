...
var agendaFilter = "all";
var agendaFilterPlaza = "all";
var agendaFilterRangeDays = "all";
...
async function loadAgenda() {
  try {
    let response = await fetch("./content/agenda/agenda.json");
    if (!response.ok) {
      const alt = await fetch("/content/agenda/agenda.json");
      if (!alt.ok) throw new Error("No se pudo cargar agenda.json");
      response = alt;
    }
    ...
    if (Array.isArray(data)) {
      ...
      entries = data.items;
    } else if (data && Array.isArray(data.agenda)) {
      entries = data.agenda;
    }
    const sorted = entries.length
      ? [...entries].sort((a, b) => new Date(a.fecha || 0) - new Date(b.fecha || 0))
      : [];
    ...
    injectAgendaJsonLd(sorted);

    const filterContainer = document.querySelector(".agenda-filters");
    if (filterContainer && agendaContainer) {
      filterContainer.addEventListener("click", function (ev) {
        const target = ev.target;
        if (!(target instanceof HTMLElement)) return;
        if (!target.matches(".agenda-filter-btn")) return;
        const value = target.getAttribute("data-filter") || "all";
        agendaFilter = value;
        var buttons = filterContainer.querySelectorAll(".agenda-filter-btn");
        buttons.forEach(function (btn) {
          btn.classList.toggle("is-active", btn === target);
        });
        if (agendaCache) {
          renderAgendaPage(agendaCache, featuredEvent);
        }
      });
      var plazaSelect = document.getElementById("agenda-filter-plaza");
      var rangeSelect = document.getElementById("agenda-filter-range");
      if (plazaSelect) {
        plazaSelect.addEventListener("change", function () {
          agendaFilterPlaza = plazaSelect.value || "all";
          if (agendaCache) {
            renderAgendaPage(agendaCache, featuredEvent);
          }
        });
      ...
      if (rangeSelect) {
        rangeSelect.addEventListener("change", function () {
          agendaFilterRangeDays = rangeSelect.value || "all";
          if (agendaCache) {
            renderAgendaPage(agendaCache, featuredEvent);
          }
        });
...
function injectAgendaJsonLd(events) {
  try {
    if (!Array.isArray(events) || !events.length) return;
    var existing = document.getElementById("agenda-jsonld");
    if (existing) existing.remove();
    function absoluteUrl(u) {
      if (!u) return u;
      if (/^https?:\/\//i.test(u)) return u;
      var base = (typeof siteUrl === "string" && siteUrl) ? siteUrl.replace(/\/+$/,"") : "";
      if (!base) return u;
      return base + (u[0] === "/" ? u : "/" + u);
    }
    var todayStr = new Date().toISOString().slice(0,10);
    var upcoming = events
      .filter(function(e){ return e && e.fecha && e.fecha >= todayStr; })
      .slice(0, 12)
      .map(function(e){
        var name = e.titulo || e.tipo || "Festejo taurino";
        var startDate = e.fecha + (e.hora ? ("T" + e.hora) : "");
        var locName = [e.plaza, e.ciudad].filter(Boolean).join(" · ");
        var obj = {
          "@type": "Event",
          "name": name,
          "startDate": startDate,
          ...
          "location": {
            ...
            "address": e.ciudad || undefined
          },
          "organizer": { "@type": "Organization", "name": "Andanada del 5" }
        };
        if (e.poster) obj.image = [absoluteUrl(e.poster)];
        if (e.buyUrl) obj.offers = { "@type": "Offer", "url": absoluteUrl(e.buyUrl), "availability": "https://schema.org/InStock" };
        return obj;
      });
    if (!upcoming.length) return;
    var payload = { "@context": "https://schema.org", "@graph": upcoming };
...
function createPosterPlaceholder(event) {
  const cityStr = event.ciudad ? String(event.ciudad).toLowerCase() : "";
  const isSevilla = cityStr.indexOf("sevilla") !== -1;
  const isCastellon = cityStr.indexOf("castell") !== -1;
  const isValencia = cityStr.indexOf("valencia") !== -1;
  const isPopular =
    event.tipo && String(event.tipo).toLowerCase().indexOf("festejo popular") !== -1;
  const ganaderiaStr = event.ganaderia ? String(event.ganaderia).toLowerCase() : "";
  const isCuadri = ganaderiaStr.indexOf("cuadri") !== -1;
  const label = isPopular
    ...
    : (isCuadri
        ...
        : (isSevilla
            ...
            : (isCastellon
                ? "CASTELLÓN"
                : (isValencia ? "VALENCIA" : "LAS VENTAS"))));
  return `
    <div class="poster-placeholder ${
      isPopular
        ? "poster-placeholder--popular"
        : (isCuadri
            ? "poster-placeholder--cuadri"
            : (isSevilla
                ? "poster-placeholder--maestranza"
                : (isCastellon
                    ? "poster-placeholder--castellon"
                    : (isValencia ? "poster-placeholder--valencia" : "poster-placeholder--ventas"))))
    }">
...
function renderAgendaPage(events, featuredEvent) {
  ...
  if (agendaFilter && agendaFilter !== "all") {
    list = list.filter(function (e) {
      const tipo = e.tipo ? String(e.tipo).toLowerCase() : "";
      if (agendaFilter === "corridas") {
        return tipo.indexOf("corrida") !== -1;
      }
      if (agendaFilter === "novilladas") {
        return tipo.indexOf("novillada") !== -1;
      }
      if (agendaFilter === "rejones") {
        return tipo.indexOf("rejones") !== -1;
      }
      if (agendaFilter === "popular") {
        return tipo.indexOf("festejo popular") !== -1;
      }
      return true;
    });
  }

  if (agendaFilterPlaza && agendaFilterPlaza !== "all") {
    var plazaNeedle = String(agendaFilterPlaza).toLowerCase();
    list = list.filter(function (e) {
      var ciudad = e.ciudad ? String(e.ciudad).toLowerCase() : "";
      return ciudad.indexOf(plazaNeedle) !== -1;
    });
  }

  if (agendaFilterRangeDays && agendaFilterRangeDays !== "all") {
    var days = parseInt(agendaFilterRangeDays, 10);
    if (!isNaN(days) && days > 0) {
      ...
      list = list.filter(function (e) {
        ...
        return e.fecha >= todayStr && e.fecha <= endStr;
      });
    }
  }
  ...
  container.innerHTML = list
    .map((event) => {
      ...
      const isPopular =
        event.tipo &&
        String(event.tipo).toLowerCase().indexOf("festejo popular") !== -1;
      const cardClass = isPopular
        ? "agenda-card agenda-card--popular"
        : (isSevilla
            ? "agenda-card agenda-card--sevilla"
            : (isCastellon
                ? "agenda-card agenda-card--castellon"
                : (isValencia ? "agenda-card agenda-card--valencia" : "agenda-card")));
